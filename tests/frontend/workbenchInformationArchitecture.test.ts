...
Deno.test("3. the leads are the only flex-1 child of the view", () => {
  const table = read("../../src/components/chat/workspace/workbench/leadTable/LeadTable.tsx");
  assert(/flex-1 min-h-0 overflow-auto/.test(table),
    "LeadTable takes the remaining height and scrolls inside it");
});
