...
Deno.test('"ONLY seed-stage" → seed hard, carried as a hard constraint, shown as unprovable', () => {
  ...
  assertEquals(seed.status, "unprovable_today");
  const s = criteriaSections(m);
  assert(s.hard.some((l) => l.includes("seed")));
  assert(s.unsupported.some((l) => l.includes("seed") && l.includes("hard")));
});
