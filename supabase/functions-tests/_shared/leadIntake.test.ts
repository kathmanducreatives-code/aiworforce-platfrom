...
import {
  ...
  type LeadRequest,
} from "../../functions/_shared/leadIntake.ts";
...
import { buildLeadSourceSelector, leadRequestToToolInput as _toTI, type ToolAvailability, type LeadSourceType } from "../../functions/_shared/leadIntake.ts";
...
Deno.test("hasNewSourcingIntent: a lead brief beats save/refine handlers", async () => {
  const { hasNewSourcingIntent } = await import("../../functions/_shared/leadIntake.ts");
