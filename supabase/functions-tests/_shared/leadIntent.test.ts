...
import {
  ...
  type RawCandidate,
} from "../../functions/_shared/leadIntent.ts";
import { classifyRoleFamily, roleMatchesFamily, isProfileOrEquityTitle } from "../../functions/_shared/roleFamilies.ts";
...
Deno.test("icpConstraintsFromIntent: SaaS 5-150 → max 150, not enterprise, SaaS positive", async () => {
  const { icpConstraintsFromIntent } = await import("../../functions/_shared/companyIcpFilter.ts");
  const li = extractLeadIntent({
    message: "Find founders hiring executive assistants.",
