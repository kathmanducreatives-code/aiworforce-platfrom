...
import { resolveActorForSourceType } from "../../functions/_shared/actorRegistry.ts";
...
Deno.test("T32c the flag ALONE cannot enable it — an allow-list is required", async () => {
  const { applySequentialSourceExecution } = await import("../../functions/_shared/sequentialSourceBridge.ts");
