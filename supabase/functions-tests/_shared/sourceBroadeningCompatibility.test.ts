...
} from "../../functions/_shared/actorInputPlanner.ts";
import {
...
import { newSourceExecutionState, type SourceExecutionState } from "../../functions/_shared/sourceExecutionState.ts";
import { actorKeyForCapability, prepareStepCall, runtimeStateFor } from "../../functions/_shared/sequentialSourceRuntime.ts";
import { projectAvailableActions } from "../../functions/_shared/sourceFeedbackContract.ts";
import { HIRING_SOURCE_CATALOG, type HiringSourceCapabilityId } from "../../functions/_shared/hiringSourceCatalog.ts";
...
Deno.test("19. with the source-planning flag OFF nothing here runs at all", async () => {
  ...
  const { isDynamicSourcePlanningEnabled } = await import("../../functions/_shared/hiringSourcePlan.ts");
