...
import {
  ...
  type DualWriteDeps,
} from "../../functions/_shared/signalsV2DualWrite.ts";
import {
...
} from "../../functions/_shared/signalsV2Writer.ts";
...
Deno.test("observability reconciles across a mixed run (considered = attempted + skipped)", async () => {
  ...
  const { writeLeadEvidenceV2, writeSignalEventV2, writeSignalEventEvidenceV2 } = await import("../../functions/_shared/signalsV2Writer.ts");
