...
} from "../../functions/_shared/companySemanticClassification.ts";
...
Deno.test("4. compoundSourcingPipeline classifies through the canonical module", async () => {
  ...
  assert(src.includes('from "../../functions/_shared/companySemanticClassification.ts"'),
...
Deno.test("5. run-agent reaches the classifier only via semanticClassificationBinding", async () => {
  ...
  assert(src.includes('from "../../functions/_shared/semanticClassificationBinding.ts"'),
