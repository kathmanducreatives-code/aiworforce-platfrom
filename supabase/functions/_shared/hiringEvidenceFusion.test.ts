...
import { jobRecordToSignalEvent } from "./jobsSignalAdapter.ts";
import { evaluateTimingSufficiency, compileTimingRequirement } from "./timingAssessment.ts";
...
Deno.test("F38/F40/F42 the sequential runtime fuses after EVERY attempt and reports fused yield", async () => {
  const { deterministicOrderedPlan } = await import("./hiringSourcePlan.ts");
  const { newSourceExecutionState, } = await import("./sourceExecutionState.ts");
  const { sequentialJobsInvoker, actorKeyForCapability, applyObservation } =
    await import("./sequentialSourceRuntime.ts");
...
Deno.test("F43 without a fusion state the runtime behaves exactly as PR #108 shipped", async () => {
  const { deterministicOrderedPlan } = await import("./hiringSourcePlan.ts");
  const { newSourceExecutionState } = await import("./sourceExecutionState.ts");
  const { sequentialJobsInvoker, actorKeyForCapability } = await import("./sequentialSourceRuntime.ts");
