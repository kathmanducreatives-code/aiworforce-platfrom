...
export const HIRING_ACTOR_CATALOG: Readonly<Record<string, HiringActorCard>> = Object.freeze({
  ...
  apify_linkedin_company_search: {
    ...
    outputs: ["id", "name", "linkedinUrl", "website", "description",
      "employeeCountRange (full mode only — the DECLARED size band)",
      "employeeCount (full mode only — LinkedIn associated members, NOT staff)", "industries (full mode)",
    ...
    not_for: ["ICP qualification", "semantic/concept search",
      "proving industry", "proving the size band (a search row's band is plausible until the company record is read)"],
    ...
    known_defects: [
      ...
      { id: "company_search_size_filters_wrong_field",
        // RE-READ 2026-09-24 (companySize.ts). The "4/8 precision" measured the
        // declared band against the LinkedIn member count — two different facts,
        // not one mis-measured one. The filter does what it says.
        summary: "companySize filters the company's DECLARED band (employeeCountRange); employeeCount is LinkedIn associated members, a different fact (Cisco Networking Academy: declared 51-200, 4,642 members — its students).",
        mitigation: "A search row's band is plausible; the company record's declared band settles a size criterion. The member count never passes, fails or contests size.",
        evidence_ref: `${EV}/final_recommendation.md#3` },
      ...
      { id: "company_search_short_mode_no_headcount",
        summary: "short mode returns neither employeeCountRange (the declared band) nor employeeCount (members) — 108 of 108 rows on run 4250f181 — so size cannot be read in the cheap mode.",
        mitigation: "Discovery runs in full mode for a plausible band; the company record settles it.",
  ...
  apify_linkedin_company_details: {
    ...
    best_for: ["the company's declared size band (employeeCountRange) — authoritative for a band claim",
      "LinkedIn associated-member count (employeeCount) — informational, NOT staff",
      "authoritative industry id + hierarchy",
    ...
    known_defects: [
      ...
      { id: "company_details_range_contradicts_count",
        // RE-READ 2026-09-24: not a contradiction. 112 audited companies: members
        // inside the declared band for 61, above it for 22, below it for 28.
        summary: "employeeCountRange (declared band) and employeeCount (LinkedIn associated members) routinely differ, because they measure different things.",
        mitigation: "Normalizer keeps them in separate fields; the band answers size criteria, the member count answers none.",
  ...
  apify_linkedin_job_search: {
    ...
    best_for: [
      "JOB-FIRST DISCOVERY: employers with an open role right now — the employer's LinkedIn URL, website and declared size band arrive on every row (10/10 observed), so no Company Search is needed",
    ...
    known_defects: [
      ...
      { id: "job_search_no_size_filter",
        summary: "No company-size input: the same probe returned LinkedIn (declared 10,001+) and Bevi (declared 201-500) beside startups declaring 11-50.",
        mitigation: "Size is read from the employer's DECLARED band on the row (plausible) and settled by the company record — a filter only when the mission's size is hard. employeeCount on the row is LinkedIn members, never size.",
        evidence_ref: "tests/fixtures/lead-v2/p3-job-discovery-probe.json" },
