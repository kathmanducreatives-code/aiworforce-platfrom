...
export const HIRING_ACTOR_CATALOG: Readonly<Record<string, HiringActorCard>> = Object.freeze({
  ...
  apify_linkedin_company_search: {
    ...
    outputs: ["id", "name", "linkedinUrl", "website (full mode only)", "description (full mode only)",
      "employeeCount (full mode only)", "employeeCountRange", "industries (full mode)",
      "industry (short mode)", "summary (short mode)", "companyType", "locations", "specialities", "tagline"],
    ...
    known_defects: [
      ...
      { id: "company_search_industry_unreliable",
        ...
        evidence_ref: `${EV}/final_recommendation.md#3` },
      { id: "company_search_short_mode_no_website",
        summary: "Short-mode rows carry no `website` (and no `description`): 108/108 rows in Lead V2 run 4250f181 had only _meta, followers, id, industry, linkedinUrl, location, logo, name, summary, universalName.",
        mitigation: "Identity resolution buys FULL rows — a website domain is the only identity confirmation the resolver accepts.",
        evidence_ref: "production run 4250f181 (2026-09-13), 13 identity searches, 0 resolved" },
