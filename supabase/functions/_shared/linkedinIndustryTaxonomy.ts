// THE LINKEDIN INDUSTRY TAXONOMY, FROM ITS SOURCE.
//
// Generated from the list the Actor's own input schema names as authoritative:
//   https://github.com/HarvestAPI/linkedin-industry-codes-v2
//   linkedin_industry_code_v2_all_eng_with_header.csv  (434 codes)
//
// Regenerate from that CSV; do not hand-edit ids. `104 Staffing and Recruiting`
// here agrees with `STAFFING_INDUSTRY_IDS` in `companyAggregatorEvidence.ts`,
// which was derived independently from observed enrichment output — the two
// sources concur, which is why that constant is left where it is rather than
// re-pointed at this file.
//
// WHAT THIS IS FOR, AND WHAT IT IS NOT.
//
// These ids narrow a DISCOVERY search to a plausible candidate population. They
// are not proof of anything. The Actor's verified card records that
// `industryIds:['4']` returned TechCrunch, Entrepreneur Media and a staffing
// firm, so provider industry stays a hint and `company_enrichment` remains the
// authority on a company's real industry — exactly as `not_for: "proving
// industry"` says.
//
// Pure data. No network, no provider, no model call.

/** id -> canonical English label, straight from the source CSV. */
export const LINKEDIN_INDUSTRIES: ReadonlyArray<readonly [string, string]> = Object.freeze([
...
/** Lowercased label -> id, for exact-label resolution. */
export const LINKEDIN_INDUSTRY_BY_LABEL: ReadonlyMap<string, string> = new Map(
  LINKEDIN_INDUSTRIES.map(([id, label]) => [label.toLowerCase(), id]),
);

export function linkedinIndustryLabel(id: string): string | null {
  for (const [i, label] of LINKEDIN_INDUSTRIES) if (i === id) return label;
  return null;
}
