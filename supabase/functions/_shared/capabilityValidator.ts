...
export function validateAgainstCapabilities(decision: WorkflowDecision): ValidationResult {
  ...
  if (d.selected_actor_key) {
    ...
    // Clamp max_results to actor's cap up front so it applies even when the
    // actor turns out to be disabled (the returned decision stays sane).
    ...
    if (!isActorRuntimeEnabled(actor)) {
      // url_analysis special-case: Firecrawl missing → firecrawl_unavailable.
