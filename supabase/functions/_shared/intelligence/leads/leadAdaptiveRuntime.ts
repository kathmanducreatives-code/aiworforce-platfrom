...
export interface ResolvedOrderedPlan {
  ...
  modelCalled: boolean;
  /**
   * The query packs the returned steps reference.
   *
   * An OUTPUT of the strategy, never a prerequisite for calling it. The feedback
   * loop consumes these, so production never supplies packs as a fixture.
   */
  packs: QueryPack[];
  /** Safe provenance from the validator, for persistence. */
...
export interface ResolveStrategyInput {
  ...
  validate: (raw: unknown) => {
    ...
    /** Packs generated by the strategy. Referenced by the returned steps. */
    packs?: QueryPack[];
    diagnostics?: Record<string, unknown>;
  };
...
export async function resolveAdaptiveOrderedPlan(
  ...
  const fallback = (reason: string, modelCalled = false): ResolvedOrderedPlan => ({
    steps: [], strategySource: "deterministic_fallback", fallbackReason: reason,
    modelCalled, packs: [], diagnostics: { fallback_reason: reason },
  });
  ...
  return {
    steps, strategySource: v.source, fallbackReason: null, modelCalled: true,
    packs: [...(v.packs ?? v.strategy.query_packs ?? [])],
