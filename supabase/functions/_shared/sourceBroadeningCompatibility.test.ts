...
import { newSourceExecutionState, type SourceExecutionState } from "./sourceExecutionState.ts";
import { actorKeyForCapability, prepareStepCall, runtimeStateFor } from "./sequentialSourceRuntime.ts";
import { projectAvailableActions } from "./sourceFeedbackContract.ts";
import { HIRING_SOURCE_CATALOG, type HiringSourceCapabilityId } from "./hiringSourceCatalog.ts";
...
Deno.test("19. with the source-planning flag OFF nothing here runs at all", async () => {
  ...
  const { isDynamicSourcePlanningEnabled } = await import("./hiringSourcePlan.ts");
