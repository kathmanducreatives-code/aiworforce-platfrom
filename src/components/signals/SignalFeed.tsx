...
import { RefreshCw, Inbox, ListOrdered } from "lucide-react";
...
export default function SignalFeed() {
  ...
  return (
    <div className="p-4 space-y-3 text-[#C9D1D9]">
      {/* Header */}
      ...
      <div className="flex items-center gap-1 flex-wrap border-b border-white/[0.06] pb-2">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`text-[12px] px-2.5 py-1 rounded-md ${tab === t.key ? "bg-emerald-500/10 border border-emerald-500/30 text-emerald-300" : "text-neutral-400 hover:text-neutral-200"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab !== "drafts" && tab !== "saved" && (
        <div className="flex items-center gap-2 flex-wrap">
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search signals…"
            className="text-[12px] px-2.5 py-1.5 rounded-md border border-white/[0.08] bg-white/[0.02] text-[#C9D1D9] placeholder:text-neutral-500 w-48" />
          <select value={priority} onChange={(e) => setPriority(e.target.value)}
            className="text-[12px] px-2 py-1.5 rounded-md border border-white/[0.08] bg-[#0d1117] text-[#C9D1D9]">
          ...
          <label className="text-[12px] inline-flex items-center gap-1.5 text-neutral-400">
      ...
      {/* States */}
      {error && <div className="rounded-md border border-red-500/30 bg-red-500/10 text-red-300 text-[12px] px-3 py-2">Couldn't load signals: {error}. <button onClick={() => void refresh()} className="underline">Retry</button></div>}
      {loading && <div className="text-[12px] text-neutral-500 py-6 text-center">Loading signals…</div>}
      ...
      {!loading && tab !== "drafts" && tab !== "saved" && (
        filtered.length === 0
          ? <Empty text="No saved signals yet. Ask Scout to find hiring signals, LinkedIn engagement, or competitor conversations." />
