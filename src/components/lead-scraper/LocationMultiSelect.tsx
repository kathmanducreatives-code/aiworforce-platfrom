...
export const LocationMultiSelect = ({
  ...
  return <div className={cn("relative transition-all duration-300 ease-out-quart rounded-xl border", isActive ? "bg-[#161616] border-[#00FF85] shadow-[0_0_20px_rgba(0,255,133,0.2)] z-50 scale-[1.02]" : "bg-[#161616] border-[#00FF85]/30 hover:border-[#00FF85]/60 hover:shadow-[0_0_15px_rgba(0,255,133,0.1)]")}>
      {/* Header / Trigger Area */}
      ...
      inputRef.current?.focus();
        ...
        <div className="flex flex-wrap gap-2 min-h-[32px]">
          {value.map(val => <Badge key={val} variant="secondary" className="bg-[#00FF85]/10 text-[#00FF85] border border-[#00FF85]/30 px-2 py-1 h-8 text-sm gap-1 hover:bg-[#00FF85]/20 transition-colors animate-in fade-in zoom-in-50 duration-200">
              {getLabel(val)}
              <button type="button" className="ml-1 rounded-full p-0.5 hover:bg-[#00FF85]/20 hover:text-white transition-colors" onClick={e => handleRemove(val, e)}>
                <X className="w-3 h-3" />
              </button>
            </Badge>)}
          <input ref={inputRef} type="text" className="bg-transparent border-none outline-none text-white placeholder:text-muted-foreground/50 h-8 min-w-[150px] flex-1 text-sm font-medium" placeholder={value.length === 0 ? placeholder : ""} value={search} onChange={e => setSearch(e.target.value)} onFocus={() => setIsActive(true)} onBlur={() => setTimeout(() => setIsActive(false), 200)} // Delay for click handling
        />
        </div>
      </div>

      {/* Sliding Dropdown Content */}
      <div className={cn("overflow-hidden transition-all duration-300 ease-out-quart", isActive ? "max-h-[300px] border-t border-[#00FF85]/20 opacity-100 p-2" : "max-h-0 opacity-0 border-t-0 p-0")}>
        <div className="max-h-[290px] overflow-y-auto scrollbar-thin scrollbar-thumb-[#00FF85]/20 scrollbar-track-transparent pr-1">
          ...
          filteredRegions.map(region => {
            ...
            return <div key={region.id} className="mb-2">
                    ...
                    {/* Countries */}
                    {matchingCountries.map(country => <div key={country} className={cn("flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer transition-all ml-2", value.includes(country) ? "bg-[#00FF85]/10 text-[#00FF85]" : "text-muted-foreground hover:bg-white/5 hover:text-white")} onMouseDown={e => {
                e.preventDefault(); // Prevent blur
                handleSelect(country);
              }}>
                        <div className={cn("w-4 h-4 rounded-sm border flex items-center justify-center transition-colors", value.includes(country) ? "border-[#00FF85] bg-[#00FF85]" : "border-muted-foreground/40")}>
      ...
      <div className={cn("absolute bottom-0 left-0 h-[2px] bg-[#00FF85] transition-all duration-500 ease-out", isActive ? "w-full shadow-[0_-2px_10px_rgba(0,255,133,0.5)]" : "w-0")} />
    </div>;
};