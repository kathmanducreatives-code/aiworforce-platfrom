        ...
        {tab === "scribe" ? (
              ...
              {SCRIBE_ACTIONS.map((a) => {
                ...
                return (
                  <button key={a.id} disabled={disabled}
                    ...
                    className="flex w-full items-center gap-2.5 rounded-lg border border-transparent px-3 py-2 text-left text-[13px] text-foreground/85 transition-colors hover:border-white/[0.06] hover:bg-emerald-500/[0.04] disabled:cursor-not-allowed disabled:text-muted-foreground/40 disabled:hover:border-transparent disabled:hover:bg-transparent">
                    {busy === a.id
            ...
            <form
              ...
              <div className="flex justify-end px-2 pb-2">
                <button type="submit" disabled={!!blocked || !ask.trim() || !!busy} aria-label="Send to Scribe"
                  className={`inline-flex h-7 w-7 items-center justify-center rounded-md border ${ACCENT.accentBorder} ${ACCENT.accentBg} ${ACCENT.accentText} hover:bg-emerald-500/15 disabled:border-white/[0.06] disabled:bg-transparent disabled:text-muted-foreground/40`}>
          ...
          <div className="space-y-6">
            {!item ? (
              ...
              <>
                <section>
                  <p className="mb-2 text-[12px] font-medium text-muted-foreground/70">Versions</p>
                        ...
                        <li key={v.id}>
                          <button onClick={() => onPreview(isCurrent || selected ? null : v)}
                            ...
                            <span className="flex items-center gap-2 text-[12.5px]">
                              ...
                              {isCurrent && <span className={`text-[11px] ${ACCENT.accentText}`}>current</span>}
                ...
                <section>
                  <p className="mb-2 text-[12px] font-medium text-muted-foreground/70">Images</p>
                        ...
                        <li key={a.id} className="min-w-0">
                          {a.url
                            ? <img src={a.url} alt={isCurrent ? "Current image" : "Earlier image"} loading="lazy"
                                className={`aspect-square w-full rounded-lg border object-cover ${isCurrent ? "border-emerald-500/40" : "border-white/[0.06]"}`} />
                            : <div className="flex aspect-square w-full items-center justify-center rounded-lg border border-dashed border-white/[0.08] bg-white/[0.02] p-2 text-center text-[10.5px] leading-snug text-muted-foreground/55" title={a.failure_reason ?? undefined}>
