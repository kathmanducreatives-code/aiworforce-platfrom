...
const OldVsNewComparison = () => {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen overflow-hidden bg-zinc-50/50">
      <div className="h-screen flex flex-col lg:flex-row items-center justify-center gap-6 lg:gap-8 px-4 lg:px-8">
        ...
        <div ref={oldCardRef} className="flex-1 max-w-sm w-full opacity-0">
          <div className="bg-white/80 backdrop-blur-xl border border-zinc-200/60 rounded-2xl p-7 shadow-[0_4px_24px_rgba(0,0,0,0.04)]">
            <h3 className="font-sans font-bold text-2xl text-zinc-400 mb-6 tracking-wide uppercase">The Old Way</h3>
            <div className="space-y-4">
              {oldSteps.map((step, i) => (
                <div key={i} ref={el => { oldItemRefs.current[i] = el; }} className="flex items-start gap-3 opacity-0">
                  <X className="w-5 h-5 text-zinc-400 mt-0.5 shrink-0" />
                  <p className="font-sans text-sm text-zinc-500">{step}</p>
                </div>
              ))}
            </div>
            <p className="font-mono text-xs text-zinc-400 mt-6 tracking-wide">COST: €82,000/year + 340 hours</p>
          </div>
        </div>
        ...
        <div className="flex-shrink-0 flex flex-col items-center">
          <div className="relative w-[280px] h-[280px] md:w-[360px] md:h-[360px]">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 300 300">
              <circle cx="150" cy="150" r="140" fill="none" stroke="rgba(0,0,0,0.06)" strokeWidth="8" />
              <circle cx="150" cy="150" r="140" fill="none" stroke="#059669" strokeWidth="8" strokeLinecap="round"
                strokeDasharray={circumference} strokeDashoffset={dashoffset}
                style={{ transition: 'stroke-dashoffset 0.15s linear', filter: isComplete ? 'drop-shadow(0 0 16px rgba(5,150,105,0.4))' : 'none' }}
              />
            </svg>
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              {!isComplete && <span className="font-mono text-2xl md:text-3xl mt-2 text-emerald-600 font-bold tabular-nums">{progress}%</span>}
            </div>
          </div>
          <div className="mt-6">
            <div className={`rounded-full px-5 py-2.5 text-center transition-all duration-500 ${isComplete ? 'bg-emerald-600 shadow-[0_0_30px_rgba(5,150,105,0.3)]' : 'bg-white/80 backdrop-blur-md border border-zinc-200/60 shadow-sm'}`}>
              {isComplete ? (
                <p className="font-mono text-sm font-bold text-white tracking-wider">AGENCY DEPENDENCY: 100% ELIMINATED</p>
              ) : (
                <p className="font-mono text-sm text-zinc-500 tracking-wider">AGENCY DEPENDENCY: <span className="text-zinc-900 font-bold tabular-nums">{dependency}%</span></p>
              )}
            </div>
          </div>
        </div>
        ...
        <div ref={newCardRef} className="flex-1 max-w-sm w-full opacity-0">
          <div className="bg-white/80 backdrop-blur-xl border-l-4 border-l-emerald-500 border border-emerald-200/40 rounded-2xl p-7 shadow-[0_4px_24px_rgba(5,150,105,0.06)]">
            <h3 className="font-sans font-bold text-2xl text-emerald-600 mb-6 tracking-wide uppercase">The ScreeningPilot Way</h3>
            <div className="space-y-4">
              {newSteps.map((step, i) => (
                <div key={i} ref={el => { newItemRefs.current[i] = el; }} className="flex items-start gap-3 opacity-0">
                  <Check className="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                  <p className="font-sans text-sm text-zinc-600">{step}</p>
                </div>
              ))}
            </div>
            <p className="font-mono text-xs text-emerald-600 mt-6 tracking-wide">COST: €149/month. Unlimited hires. That's it.</p>
          </div>
        </div>
      </div>
    </section>
