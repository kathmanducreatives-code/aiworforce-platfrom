...
const COLUMNS = [
  {
    ...
    body: "Pilot runs in the cloud with zero regional restrictions. Whether you are building in Bangalore, building in Berlin, or building in Brooklyn — your AI team shows up at full power every single day.",
  },
  {
    Icon: Languages,
    title: "Speaks your market",
    body: "Your Company Brain stores your market context. Your agents write outreach for US investors and Indian founders differently — because they know who you are talking to and what resonates in each market.",
  },
  {
    Icon: Shield,
    title: "Trusted globally",
    body: "GDPR compliant for European founders. SOC2 ready for enterprise customers. Encrypted at rest and in transit. Your data never leaves your control regardless of where you are building.",
  },
];

// Simplified world map continents — minimal SVG paths
      ...
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-display font-black text-3xl md:text-5xl text-white leading-[1.1] mb-6">
            One founder in Mumbai. One in Berlin.<br />One in São Paulo. One in Toronto.<br />Same AI team. Same results.
          </h2>
          <p className="text-white/40 text-lg max-w-[600px] mx-auto leading-relaxed">
            Pilot works in every timezone, every language, and every startup ecosystem on earth. Your AI team never sleeps, never takes a holiday, and never needs a local office.
          </p>
        </div>
        ...
        >
          <svg viewBox="0 0 100 80" className="w-full h-auto" preserveAspectRatio="xMidYMid meet">
            ...
            {HUBS.slice(0, -1).map((hub, i) => {
              ...
              return (
                <line key={`conn-${i}`} x1={hub.x} y1={hub.y} x2={next.x} y2={next.y}
                  stroke="white" strokeWidth="0.15" opacity="0.08" />
              );
            })}
            ...
            {HUBS.map((hub, i) => (
              <g key={hub.name}>
                ...
                <text x={hub.x} y={hub.y + 2.5} textAnchor="middle" fill="white" fontSize="1.8" opacity="0.25"
                  className="hidden md:block" style={{ display: i < 5 || window.innerWidth > 768 ? undefined : 'none' }}>
                  {hub.name}
                </text>
              </g>
            ))}
        ...
        <p className="text-center text-sm text-white/30 mt-6">
          Join founders from <span className="text-white/60 font-semibold">50+ countries</span>
        </p>
      </div>
    </section>
  );
