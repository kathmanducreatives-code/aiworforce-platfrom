...
export default function WorkbenchHeader({
  ...
  const truncatedRequest = rawTaskTitle && rawTaskTitle.length > 110
    ? rawTaskTitle.slice(0, 107) + '…'
    : rawTaskTitle;
  const showRequestLine = !!panel && !!truncatedRequest;

  return (
    <div className="relative px-4 pt-3 pb-2.5 border-b border-white/[0.06] bg-gradient-to-b from-white/[0.025] via-white/[0.01] to-transparent backdrop-blur-xl shrink-0">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="text-[10px] uppercase tracking-[0.18em] text-emerald-300/70 font-medium">
            Workbench
          </div>
          <h2 className="mt-0.5 text-[15px] text-[#F0F6FC] font-medium leading-snug truncate" title={conciseTitle}>
            {conciseTitle}
          </h2>
          {subtitle && (
            <div className="mt-0.5 text-[12px] text-[#7D8590] truncate" title={subtitle}>{subtitle}</div>
          )}
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className={cn(
              'h-7 w-7 inline-flex items-center justify-center rounded-md hover:bg-white/[0.06] text-[#7D8590] hover:text-[#F0F6FC] transition-colors',
          ...
          <button
            ...
            className="h-7 w-7 inline-flex items-center justify-center rounded-md hover:bg-white/[0.06] text-[#7D8590] hover:text-[#F0F6FC] transition-colors"
          >
      ...
      <div className="mt-2 flex items-center gap-1.5 flex-wrap text-[11px]">
        ...
        {provider && (
          <span className="text-[10.5px] px-2 h-5 inline-flex items-center rounded-md border border-white/[0.07] bg-white/[0.03] text-[#C9D1D9]">
            {provider}
          </span>
        )}
        <span className="text-[10.5px] px-2 h-5 inline-flex items-center rounded-md border border-emerald-500/20 bg-emerald-500/[0.06] text-emerald-200">
          {workflowLabel}
        </span>
        <span className={`text-[10.5px] px-2 h-5 inline-flex items-center rounded-md border capitalize ${STATUS_TONE[status] ?? STATUS_TONE.pending}`}>
          {status.replace('_', ' ')}
        </span>
        {apifyTotal !== null && (
          <span className="text-[10.5px] px-2 h-5 inline-flex items-center rounded-md border border-white/[0.07] bg-white/[0.02] text-[#C9D1D9]">
            {apifyTotal} result{apifyTotal === 1 ? '' : 's'}
          </span>
        )}
        {ts && (
          <span className="text-[10.5px] px-2 h-5 inline-flex items-center rounded-md border border-white/[0.06] bg-white/[0.02] text-[#7D8590]">
            Updated {new Date(ts).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          </span>
        )}
        {agentSlug === 'penn' && (
          <span className="text-[10px] px-2 h-5 inline-flex items-center rounded-md border border-amber-500/30 bg-amber-500/10 text-amber-300 uppercase tracking-wider">
            approval required
          </span>
        )}
        {toolLabel && (
          <span className="font-mono text-[10px] px-2 h-5 inline-flex items-center rounded-md border border-white/[0.06] bg-white/[0.02] text-[#7D8590]">
            {toolLabel}
          </span>
        )}
      </div>
      ...
      {showRequestLine && (
        <div className="mt-2 text-[11px] text-[#7D8590] truncate" title={rawTaskTitle ?? undefined}>
          <span className="text-[#484F58]">Request:</span> {truncatedRequest}
        </div>
      )}
    </div>
  );
