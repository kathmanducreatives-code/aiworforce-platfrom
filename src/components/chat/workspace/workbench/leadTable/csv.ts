...
export function rowsToCsv(rows: LeadTableRow[]): string {
  const headers = [
    ...
    'analyst_verdict', 'scout_rank', 'scout_pre_rank_score', 'why_now', 'icp_fit_summary', 'evidence_summary', 'risk_flags', 'recommended_next_action', 'outreach_angle',
  ...
  for (const r of rows) {
    ...
    lines.push([
      ...
      esc(raw.analyst_verdict),
      esc(raw.scout_rank),
      esc(raw.scout_pre_rank_score),
      esc(raw.why_now),
      esc(raw.icp_fit_summary),
      esc(raw.evidence_summary),
      esc(arr(raw.risk_flags)),
      esc(raw.recommended_next_action),
      esc(raw.outreach_angle),
