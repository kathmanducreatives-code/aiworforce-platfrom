  ...
  try {
    ...
    for (const comp of competitors) {
      try {
        ...
        let markdown = '';
        let pricingUrl = '';
        for (const path of ['/pricing', '/plans', '/price']) {
          try {
            pricingUrl = `${baseUrl}${path}`;
            const res = await firecrawl.scrapeUrl(pricingUrl, {
              formats: ['markdown'],
              onlyMainContent: true,
            });
            markdown = res?.data?.markdown || res?.markdown || '';
            if (markdown && markdown.length > 100) break;
            await delay(500);
          } catch {
            continue;
          }
        ...
        const currentData = { raw_content: markdown.slice(0, 8000), scraped_url: pricingUrl };
        const changeDetected = previousData
        ...
        if (changeDetected) {
          await (supabase as any).from('competitor_intel_signals').insert({
            ...
            signal_data: { before_id: latestHistory?.[0]?.id, current: currentData },
