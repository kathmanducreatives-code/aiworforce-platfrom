// DISPLAY MIRROR of supabase/functions/_shared/creditPricing.ts.
//
// THE EDGE MODULE IS THE AUTHORITY — it is what `authorizeProviderCall` is
// given, so it is what actually reserves and charges. This copy exists only so
// the Workbench can show a price before the request is made, and
// `tests/frontend/creditPricingMirror.test.ts` pins the two tables equal.
//
// A price shown by one module and charged by another is the `~Nc` badge this
// codebase already had to correct once — a number on a button that no ledger
// ever honoured. If these drift, the test fails before a user is misquoted.
//
// ── WHY THIS EXISTS ─────────────────────────────────────────────────────────
//
