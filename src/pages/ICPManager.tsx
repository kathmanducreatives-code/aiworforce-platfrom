        ...
        <div className="min-h-screen bg-background">
            <div className="max-w-[1280px] mx-auto px-6 lg:px-8 py-6 space-y-5">
                <PageHeader
                    ...
                    subtitle="Manage your Ideal Customer Profile search strategies"
                    breadcrumbs={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'ICP Intelligence' }]}
                    primaryAction={{
                        label: 'Create Profile',
                        onClick: () => setIsCreateDialogOpen(true),
                        icon: <Plus className="w-4 h-4" />,
                    }}
                    secondaryActions={sessions.length > 0 ? [{
                        label: selectionMode ? 'Cancel' : 'Select',
                        onClick: () => { setSelectionMode(!selectionMode); setSelectedSessions(new Set()); },
                        icon: selectionMode ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />,
                    }] : []}
                />
                ...
                {sessions.length > 0 && (
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <MetricCard label="Sessions" value={stats.totalSessions} icon={<Target className="w-4 h-4 text-primary" />} />
                        <MetricCard label="Candidates" value={stats.totalCandidates} icon={<Users className="w-4 h-4 text-blue-500" />} />
                        <MetricCard label="Avg Match" value={`${stats.avgMatchScore}%`} icon={<TrendingUp className="w-4 h-4 text-emerald-500" />} />
                        <MetricCard label="Strong" value={stats.totalStrong} icon={<Zap className="w-4 h-4 text-amber-500" />} />
                    </div>
                )}
